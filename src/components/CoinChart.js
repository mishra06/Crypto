...
const CoinChart = ({ currency }) => {
  ...
  const prevParamsRef = useRef({ id, currency, days });
  ...
  useEffect(() => {
    ...
    if (prevParams.id !== id || prevParams.currency !== currency || prevParams.days !== days) {
      GetData();
      prevParamsRef.current = { id, currency, days };
    }
  }, [GetData, id, currency, days]);
